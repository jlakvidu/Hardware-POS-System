...
const closeResetPopup = () => {
  ...
  otpDigits.value = ['', '', '', '', '', '']
  // Clear resend timer if popup is closed
  if (resendInterval) {
...
const handleResendCode = async () => {
  ...
  resetError.value = ''
  canResend.value = false // Immediately hide the button and show timer
  startResendTimer() // Start timer immediately on click
  ...
  try {
    ...
    resetError.value = error.response?.data?.error || 'Failed to resend code'
    // If error, allow resend again
    canResend.value = true
    if (resendInterval) {
      clearInterval(resendInterval)
      resendInterval = null
            ...
            <div v-else-if="resetStep === 2" class="space-y-5">
              ...
              <div class="flex justify-between items-center">
                ...
                <button 
                  ...
                  class="text-sm transition-colors flex items-center text-blue-400 hover:text-blue-300"
                >
                  <svg
                    v-if="resendLoading"
