...
const fetchLowStockItems = async () => {
    ...
    try {
        ...
        // Only keep items that are actually low stock or out of stock
        ...
        // Remove old low-stock notifications for products that are no longer low stock
        const lowStockIds = new Set(items.map(i => i.id))
        notifications.value = [
            ...notifications.value.filter(n => n.type !== 'low-stock' || lowStockIds.has(n.id)),
            ...items.filter(i => !notifications.value.some(n => n.id === i.id))
        ]
    } catch (error) {
...
const toggleNotifications = () => {
    ...
    if (showNotifications.value) {
        ...
        refreshNotifications() // Immediately refresh notifications when opening
    }
}
...
const refreshNotifications = async () => {
    notificationsLoading.value = true
    // Remove all previous low-stock notifications and fetch new ones
    notifications.value = notifications.value.filter(n => n.type !== 'low-stock')
            ...
            <div class="flex justify-between items-center h-20">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-2xl font-bold text-white tracking-tight">
                        Weads Horana 
                        <span class="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-cyan-400 font-extrabold">Pvt Ltd</span>
                    </h1>
                </div>
                    ...
                    <div class="relative notifications-container">
                        ...
                        <div v-if="showNotifications" class="notification-dropdown absolute right-0 top-14 w-[480px] bg-gray-800 rounded-xl shadow-2xl border border-blue-900/50 overflow-hidden">
                            ...
                            <div class="max-h-[480px] overflow-y-auto">
                                <!-- Loading spinner -->
                                <div v-if="notificationsLoading" class="flex flex-col items-center justify-center py-12">
                                    ...
                                    <span class="text-gray-400">Loading notifications...</span>
                                </div>
                                <div v-else>
